...
conn_paramss = pika.ConnectionParameters("localhost", virtual_host="/", credentials=creds_broker)

conn_broker = pika.BlockingConnection(conn_paramss)
...
def api_ping (channel, method, header, body):
    ...
    channel.basic_publish(body="Pong!{}".format(msg_dict["time"]),exchange='',routing_key=header.reply_to)
